...
florida_stats = np.zeros((florida_x.shape[0], florida_y.shape[0], 7, len(dates)))
...
for i in range(len(reduced_file_list)):
	...
	if(any(day_ahead >= 0 and day_ahead <= 8 for day_ahead in days_ahead)):
		...
		day_inds = [day_ahead >= 0 and day_ahead <= 8 for day_ahead in days_ahead]
		...
		florida_stats[np.tile(x_ind, day_inds.shape[0]), np.tile(y_ind, day_inds.shape[0]), 0, np.repeat(day_inds, x_ind.shape[0])] += np.tile(angstrom[avail_inds], day_inds.shape[0])

		avail_inds = ~np.isnan(chlor_a)
		x_ind = find_nearest_batch(florida_x, longarr[avail_inds])
		y_ind = find_nearest_batch(florida_y, latarr[avail_inds])
		florida_stats[np.tile(x_ind, day_inds.shape[0]), np.tile(y_ind, day_inds.shape[0]), 1, np.repeat(day_inds, x_ind.shape[0])] += np.tile(chlor_a[avail_inds], day_inds.shape[0])

		avail_inds = ~np.isnan(chl_ocx)
		x_ind = find_nearest_batch(florida_x, longarr[avail_inds])
		y_ind = find_nearest_batch(florida_y, latarr[avail_inds])
		florida_stats[np.tile(x_ind, day_inds.shape[0]), np.tile(y_ind, day_inds.shape[0]), 2, np.repeat(day_inds, x_ind.shape[0])] += np.tile(chl_ocx[avail_inds], day_inds.shape[0])

		avail_inds = ~np.isnan(Kd_490)
		x_ind = find_nearest_batch(florida_x, longarr[avail_inds])
		y_ind = find_nearest_batch(florida_y, latarr[avail_inds])
		florida_stats[np.tile(x_ind, day_inds.shape[0]), np.tile(y_ind, day_inds.shape[0]), 3, np.repeat(day_inds, x_ind.shape[0])] += np.tile(Kd_490[avail_inds], day_inds.shape[0])

		avail_inds = ~np.isnan(poc)
		x_ind = find_nearest_batch(florida_x, longarr[avail_inds])
		y_ind = find_nearest_batch(florida_y, latarr[avail_inds])
		florida_stats[np.tile(x_ind, day_inds.shape[0]), np.tile(y_ind, day_inds.shape[0]), 4, np.repeat(day_inds, x_ind.shape[0])] += np.tile(poc[avail_inds], day_inds.shape[0])

		avail_inds = ~np.isnan(nflh)
		x_ind = find_nearest_batch(florida_x, longarr[avail_inds])
		y_ind = find_nearest_batch(florida_y, latarr[avail_inds])
		florida_stats[np.tile(x_ind, day_inds.shape[0]), np.tile(y_ind, day_inds.shape[0]), 5, np.repeat(day_inds, x_ind.shape[0])] += np.tile(nflh[avail_inds], day_inds.shape[0])

		florida_stats[np.tile(x_ind, day_inds.shape[0]), np.tile(y_ind, day_inds.shape[0]), 6, np.repeat(day_inds, x_ind.shape[0])] += 1
	
features = np.zeros((florida_stats.shape[0], florida_stats.shape[1], 7, len(dates)))
for i in range(florida_stats.shape[0]):
	for j in range(florida_stats.shape[1]):
		for k in range(len(dates)):
			if(florida_stats[i, j, 6, k] == 0):
				features[i, j, :, k] = -1
			else:
				features[i, j, 0, k] = florida_stats[i, j, 0, k]/florida_stats[i, j, 6, k]
				features[i, j, 1, k] = florida_stats[i, j, 1, k]/florida_stats[i, j, 6, k]
				features[i, j, 2, k] = florida_stats[i, j, 2, k]/florida_stats[i, j, 6, k]
				features[i, j, 3, k] = florida_stats[i, j, 3, k]/florida_stats[i, j, 6, k]
				features[i, j, 4, k] = florida_stats[i, j, 4, k]/florida_stats[i, j, 6, k]
				features[i, j, 5, k] = florida_stats[i, j, 5, k]/florida_stats[i, j, 6, k]
...
features = np.reshape(features, (features.shape[0]*features.shape[1], 7, len(dates)))
...
for i in range(len(orig_indices_bedrock)):
	for j in range(len(dates)):
		features[i, 6, j] = bedrock_z[orig_indices_bedrock[i][0]][orig_indices_bedrock[i][1]]

land_mask = features[:,6,0] > 0
...
for day in dates:
	...
	configfilename = 'date_train_test_depth_norm_w_knn_ca'
	...
	np.save('map_images/red_tide_output{}.png'.format(day_counter), red_tide_output)
	...
	plt.gca().invert_yaxis()
	plt.savefig('map_images/red_tide_combined{}.png'.format(str(day_counter).zfill(5)), bbox_inches='tight')
